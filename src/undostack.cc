...
bool UndoAction::undo(const SigC::Slot0<bool>& pulse)
{
...
void UndoStack::undo_step(const SigC::Slot0<bool>& pulse)
{
  try
  {
    bool skip = false;
...
bool UndoStack::do_undo(const SigC::Slot0<bool>& pulse)
{
  ...
  while (!actions_.empty())
  {
    ...
    if (!skip_this)
    {
      ...
      if ((++iteration % PULSE_INTERVAL) == 0 && SigC::Slot0<bool>(pulse)())
        throw StopUndo();
    }
  }
