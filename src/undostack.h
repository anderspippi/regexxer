...
public:
  ...
  bool undo(const SigC::Slot0<bool>& pulse);

private:
  UndoAction(const UndoAction&);
  UndoAction& operator=(const UndoAction&);

  virtual bool do_undo(const SigC::Slot0<bool>& pulse) = 0;
};
...
public:
  ...
  void undo_step(const SigC::Slot0<bool>& pulse);

private:
  std::stack<UndoActionPtr> actions_;

  virtual bool do_undo(const SigC::Slot0<bool>& pulse);
};
